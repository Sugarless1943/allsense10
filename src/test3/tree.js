...
let sum = 0
data.forEach(item => {
    if((item.Type == "H" || item.Type == "V") && station[item.Parent_Tree]) {
        if(item.Type == "H") {
            sum++
            if(!names.includes(item.ID)) {
                console.log(item.ID)
            }
            station[item.Parent_Tree].push(item.ID)
        }
...
res.push({"name": "S", "children": ["S1", "S2"]})
